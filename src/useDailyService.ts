...
export function useDailyService(
  ...
  useEffect(() => {
    if (isDailyAvailable) {
      setDailyConfig(settings.apiKey, settings.model, settings.speak, settings.brain);
    }
  }, [settings.apiKey, settings.model, settings.speak, settings.brain]);
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      if (!canAnswer(settings.brain, settings.apiKey)) {
        setError(
          settings.brain === 'device'
            ? '先に右上の「設定」で、端末内AIのモデルをダウンロードしてください。'
            : '先に右上の「設定」で Gemini API キーを入力してください。'
        );
        return;
      }
